...
export const NoteViewer: React.FC<NoteViewerProps> = ({
  ...
  /**
   * Shared term hits (diagram nodes) reach the web explainer with their
   * element, so the modal can morph from the pressed node. The native host
   * drops the element when it builds the bridge payload — DOM info stops at
   * the Expo DOM/native boundary, not between DOM components.
   */
  const handleTermActivate = useCallback(
    (term: string, context: string, element?: HTMLElement | SVGElement) => {
      if (onHighlightTerm) onHighlightTerm(term, context, element as HTMLElement);
    },
    [onHighlightTerm],
  );
