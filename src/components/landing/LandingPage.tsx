...
export const LandingPage: React.FC<LandingPageProps> = ({ onOpenApp }) => (
  <div className="min-h-screen bg-[#FAF8F5] text-[#0F172A] font-body">
    <a
      href="#main"
      className="sr-only focus:not-sr-only focus:fixed focus:top-3 focus:left-3 focus:z-50 focus:px-3 focus:py-2 focus:bg-[#FDE047] focus:border focus:border-[#0F172A] focus:rounded-md focus:text-sm focus:font-semibold"
    >
      Skip to content
    </a>
...
const Brand: React.FC = () => (
  <span className="flex items-center gap-2 shrink-0">
    <span className="w-8 h-8 bg-[#0F172A] text-[#FAF8F5] rounded-lg flex items-center justify-center font-ethiopic font-bold text-sm shadow-neo-xs">
      ተ
    </span>
...
const primaryButton =
  'btn-neo inline-flex items-center justify-center gap-2 rounded-lg border-2 border-[#0F172A] bg-[#0F172A] text-[#FAF8F5] px-5 h-11 text-sm font-semibold shadow-neo whitespace-nowrap';
const secondaryButton =
  'btn-neo inline-flex items-center justify-center gap-2 rounded-lg border-2 border-[#0F172A] bg-[#FAF8F5] text-[#0F172A] px-5 h-11 text-sm font-semibold shadow-neo whitespace-nowrap';
...
const SectionHeading: React.FC<{
  ...
  <div className="max-w-2xl">
    <p className="badge-chip text-[#0D9488]">{kicker}</p>
    <h2 id={id} className="font-editorial text-3xl sm:text-4xl font-bold tracking-tight mt-2">
      {title}
    </h2>
    {children && <p className="text-base sm:text-lg text-slate-600 mt-3 leading-relaxed">{children}</p>}
  </div>
);
...
const TopBar: React.FC<{ onOpenApp: () => void }> = ({ onOpenApp }) => (
  <header className="sticky top-0 z-20 bg-[#FAF8F5]/90 backdrop-blur-md border-b border-[#0F172A]/10">
    <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
      ...
      <nav className="flex items-center gap-2 sm:gap-3" aria-label="Primary">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 px-2.5 h-9 rounded-lg text-sm font-medium text-slate-700 hover:bg-[#0F172A]/5"
        ...
        <button
          type="button"
          onClick={onOpenApp}
          className="btn-neo inline-flex items-center gap-1.5 rounded-lg border-2 border-[#0F172A] bg-[#FDE047] px-3.5 h-9 text-sm font-semibold shadow-neo-sm"
        >
          Open Temari
...
const Hero: React.FC<{ onOpenApp: () => void }> = ({ onOpenApp }) => (
  <section className="relative overflow-hidden border-b border-[#0F172A]/10" aria-labelledby="hero-title">
    ...
    <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-16 pb-20 sm:pt-24 sm:pb-28">
      ...
      <div className="max-w-3xl bg-[#FAF8F5] border-2 border-[#0F172A] rounded-2xl shadow-neo-lg p-6 sm:p-10">
        <p className="font-ethiopic text-5xl sm:text-7xl font-bold leading-none tracking-tight" lang="am">
          ተማሪ
        </p>
  ...
  <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24" aria-labelledby="loop-title">
    ...
    <ol className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      {LOOP.map((step, index) => (
        <li
          key={step.noun}
          className="relative bg-white border-2 border-[#0F172A] rounded-xl p-5 shadow-neo flex flex-col"
        >
          <div className="flex items-center justify-between">
            <span className="w-9 h-9 rounded-lg bg-[#FDE047] border-2 border-[#0F172A] flex items-center justify-center">
...
const CognitiveLevels: React.FC = () => (
  <section className="bg-white border-y border-[#0F172A]/10" aria-labelledby="levels-title">
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24 grid gap-10 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] lg:items-start">
      ...
      <ol className="grid gap-2.5">
        {BLOOM_LEVELS.map((level, index) => (
          <li
            key={level}
            className="grid grid-cols-[auto_1fr] items-center gap-4 bg-[#FAF8F5] border border-[#0F172A]/15 rounded-lg px-4 py-3"
          >
            <span
            ...
            <span className="text-sm text-slate-700">{LEVEL_EXAMPLE[level]}</span>
          </li>
        ))}
      </ol>
    </div>
    ...
    <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24" aria-labelledby="byok-title">
      ...
      <ul className="mt-8 flex flex-wrap gap-2">
        {providers.map((provider) => (
          <li
            key={provider.id}
            className="inline-flex items-center gap-2 bg-white border-2 border-[#0F172A] rounded-lg px-3.5 h-10 text-sm font-semibold shadow-neo-sm"
          >
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        <div className="bg-white border-2 border-[#0F172A] rounded-xl p-6 shadow-neo-teal">
        ...
        <div className="bg-white border-2 border-[#0F172A] rounded-xl p-6 shadow-neo-amber">
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
      ...
      <div className="flex items-center gap-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-700 hover:underline underline-offset-4"
        >
          <Github className="w-4 h-4" aria-hidden="true" />
          Repository
        </a>
