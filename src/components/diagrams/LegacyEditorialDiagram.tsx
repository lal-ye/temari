...
export interface LegacyEditorialDiagramProps {
  content: string;
  title?: string;
 ...
 */
export const LegacyEditorialDiagram: React.FC<LegacyEditorialDiagramProps> = ({
  content,
  title = 'Concept Map',
  defaultMode = 'static',
}) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`my-6 rounded-2xl border border-border/80 bg-card shadow-xs transition-all overflow-hidden ${
        isFullscreen ? 'fixed inset-4 z-50 flex flex-col bg-card shadow-lg' : ''
      }`}
    >
      ...
      <div className="px-4 py-3 bg-card border-b border-border flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-foreground uppercase tracking-wider">{parsed.title}</span>
              <span className="px-2 py-0.5 bg-muted border border-border rounded-md text-[9px] font-mono font-medium text-muted-foreground">
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors outline-hidden focus-visible:ring-2 focus-visible:ring-ring/50 ${
              teacherMode
          ...
          <button
            ...
            className="p-1.5 bg-card hover:bg-muted text-muted-foreground hover:text-foreground rounded-lg border border-border transition-colors"
            title="Copy Raw SVG"
          >
      ...
      {teacherMode && (
        <div className="px-4 py-2.5 bg-amber-500/[0.06] border-b border-amber-500/20 flex flex-wrap items-center justify-between gap-3 animate-in fade-in slide-in-from-top-1 duration-150">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 bg-foreground text-background text-[10px] font-mono font-medium rounded border border-foreground uppercase tabular-nums">
          ...
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={handlePrev}
              className="p-1 bg-card hover:bg-muted text-foreground rounded-lg border border-border transition-colors"
              title="Previous Step"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 bg-card hover:bg-muted text-foreground text-xs font-medium rounded-lg border border-border transition-colors"
            >
              {isPlaying ? <Pause className="w-3 h-3 text-rose-600 dark:text-rose-400" /> : <Play className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />}
      ...
      {teacherMode && activeStepDetails && (
        <div className="p-4 bg-card border-t border-border animate-in fade-in duration-200">
          <div className="flex items-start gap-3">
            <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-600 dark:text-amber-400 shrink-0">
              ...
              {activeStepDetails.subpoints.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {activeStepDetails.subpoints.map((sub, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 bg-muted/60 border border-border rounded-md text-[11px] font-medium text-foreground"
                    >
                      • {sub}
                    </span>
                  ))}
...
function renderSvgDiagram(parsed: ParsedDiagram, activeIndex: number | null): React.ReactElement {
  ...
  if (parsed.type === 'raw-svg') {
     ...
     */
    return (
      <div className="p-4 border border-dashed border-border rounded-xl bg-muted/40 text-center">
        <p className="text-xs font-semibold text-foreground">Raw SVG figures are not rendered</p>
        <p className="mt-1 text-[11px] font-medium text-muted-foreground max-w-md mx-auto text-pretty">
